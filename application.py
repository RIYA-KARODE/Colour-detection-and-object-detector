...
class Application(tk.Frame):
	...
	def draw_frames(self):
		...
		self.bottomBox = tk.Frame(self, width=250, height=200, bg='orange') 
	...
	def check_tracking(self):
		...
		if self.path:
			...
			messagebox.showinfo('CD object tracker', 'Enter the required value')
	...
	def track_color_from_stream(self):
		...
		if len(self.img) == 2:
			...
			messagebox.showinfo('CD object tracker', 'cannot initialize system camera')
			self.connection_option.set(0)
			self.set_selection(self.topbar.winfo_children()[2], self.features[2])
		elif self.img == "Cannot connect to stream":
			messagebox.showinfo('CD object tracker', 'Cannot connect to stream. Check your network')
			self.connection_option.set(0)
			self.set_selection(self.topbar.winfo_children()[2], self.features[2])
...
if __name__ == '__main__':
	...
	root.title('Colour detection and object tracker ')
